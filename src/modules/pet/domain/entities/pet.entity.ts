...
export class Pet {
  ...
  addVaccination(record: VaccinationRecord) {
    ...
    if (
      ...
      throw new BusinessRuleError(
        'El carnet de vacunación debe estar en formato PDF',
      );
    }
  ...
  updateVaccinationDocument(
    ...
    document: Pick<
      VaccinationRecord,
      'documentMimeType' | 'documentName' | 'documentSize'
    >,
  ) {
    const record = this.props.vaccinationRecords.find(
      (candidate) => candidate.id === vaccinationId,
    );
    if (!record) {
      throw new BusinessRuleError('Registro de vacunación no encontrado');
    }
